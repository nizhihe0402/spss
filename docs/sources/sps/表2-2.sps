...
SELECT IF ( ID是否一致 = 1). 
EXECUTE.
SAVE OUTFILE='D:\2025常见病\数据审核\XX省份\第一次反馈库\表2-2\01-表2-2-ID不一致-大学.sav' 
  /COMPRESSED.
...
EXECUTE.
SAVE OUTFILE='D:\2025常见病\数据审核\XX省份\第一次反馈库\表2-2\02-表2-2ID重复-大学.sav' 
  /COMPRESSED.
...
*表2-2大学库.
RECODE grade (41 thru 43=0)(SYSMIS=1)(ELSE=1) INTO 年级编码缺失或异常.
EXECUTE.
...
EXECUTE.
DATASET ACTIVATE  infoxx. 
SAVE OUTFILE='D:\2025常见病\数据审核\XX省份\第一次反馈库\表2-2\03-表2-2-基本信息缺失-大学.sav' 
  /COMPRESSED.
...
SELECT IF (年龄异常=1 ). 
EXECUTE.
SAVE OUTFILE='D:\2025常见病\数据审核\XX省份\第一次反馈库\表2-2\04-表2-2-年龄异常-大学.sav' 
  /COMPRESSED.
...
SAVE OUTFILE='D:\2025常见病\数据审核\XX省份\第一次反馈库\表2-2\表2-2-清理后-大学.sav' 
  /COMPRESSED.
...
SELECT IF (年龄可疑 = 1). 
EXECUTE.
SAVE OUTFILE='D:\2025常见病\数据审核\XX省份\第一次反馈库\表2-2\05-表2-2-年龄可疑-大学.sav' 
  /COMPRESSED.
...
*****5.近视-仅供大学.
RECODE VISIONR (0.1 thru 3.2 =1) (SYSMIS =1)  INTO 近视可疑.
EXECUTE.
RECODE VISIONL (0.1 thru 3.2 =1) (SYSMIS =1)  INTO 近视可疑.
EXECUTE.
RECODE 近视可疑(1=1)(ELSE=0).
EXECUTE.
VALUE LABELS 近视可疑 0'正常' 1'异常'.
FREQUENCIES VARIABLES=近视可疑
  /ORDER=ANALYSIS.

*拷贝近视可疑的数据.
DATASET COPY  jsx. 
DATASET ACTIVATE  jsx. 
FILTER OFF. 
USE ALL. 
SELECT IF (近视可疑 = 1). 
EXECUTE.
SAVE OUTFILE='D:\2025常见病\数据审核\XX省份\第一次反馈库\表2-2\06-表2-2-近视可疑-大学.sav' 
  /COMPRESSED.
...
EXECUTE.
DATASET ACTIVATE  xxx. 
SAVE OUTFILE='D:\2025常见病\数据审核\XX省份\第一次反馈库\表2-2\07-表2-2-身高体重可疑-大学.sav' 
  /COMPRESSED.
...
SELECT IF (腰围异常或缺失=1).
EXECUTE.
SAVE OUTFILE='D:\2025常见病\数据审核\XX省份\第一次反馈库\表2-2\07-表2-2-腰围可疑-大学.sav' 
  /COMPRESSED.
...
EXECUTE.
SAVE OUTFILE='D:\2025常见病\数据审核\XX省份\第一次反馈库\表2-2\08-表2-2-血压可疑-大学.sav' 
  /COMPRESSED.
...
VALUE LABELS  恒龋可疑 1'异常' 0'正常'.
FREQUENCIES VARIABLES= 恒龋可疑
  /ORDER=ANALYSIS.
...
SELECT IF (恒龋可疑=1). 
EXECUTE.
SAVE OUTFILE='D:\2025常见病\数据审核\XX省份\第一次反馈库\表2-2\09-表2-2-龋齿可疑-大学.sav' 
  /COMPRESSED.
