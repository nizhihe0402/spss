...
| `QuestionSqlParser` | Parses SQL INSERT INTO `bus_question` statements. Maps `export_content` → SPSS variable name, keyed by `table_id`; `export_sort` is only export order |
...
| Table | Purpose |
|---|---|
| `bus_question` | Question definitions; `export_content` → SPSS variable name; `export_sort` is export order; `table_id` |
